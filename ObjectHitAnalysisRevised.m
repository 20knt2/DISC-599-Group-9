...
% Read data from column BB, starting from row 2
TaskScores = readmatrix(file_path, 'Range', 'AZ2:AZ1000');
TaskScores = TaskScores(~isnan(TaskScores));  % Remove any NaNs if present
...
figure;
b = bar(TaskScores, 'FaceColor', 'flat');  % Use 'flat' to assign color per bar
...
% Define x-axis labels as "Session 1", ..., "Session N"
...
% Add asterisks above bars where TaskScore > lineY
hold on;
for i = 1:num_sessions
    if TaskScores(i) > lineY
        text(i, TaskScores(i) + 0.1, '*', 'HorizontalAlignment', 'center', 'FontSize', 20, 'Color', 'black');
    end
end

% Add title
title('Comparing Object Hit Task Scores Across Sessions');
